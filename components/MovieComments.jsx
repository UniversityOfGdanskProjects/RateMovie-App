...
const MovieComments = ({ movieId }) => {
  ...
  const editFormik = useFormik({
    ...
    validationSchema: Yup.object({
      comment: Yup.string().trim().required('Comment is required').matches(/\S/, 'Comment cannot consist of only whitespaces').max(200, "Comment can be max 200 chars long"),
    }),
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        if (response.ok) {
          setComments((prev) =>
            prev.map((comment) =>
              comment.commentId === values.commentId ? { ...comment, comment: values.comment } : comment
            )
          );
          resetForm();
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const response = await fetch(`http://localhost:7000/api/${movieId}/comments`);
        const data = await response.json();
        if (response.ok) {
          setComments(data);
        } else {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        const response = await fetch(`http://localhost:7000/api/movies/${movieId}/comment`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      <ul className='comments'>
        {comments &&
          comments.map((comment, index) => (
            <li key={`${comment.commentId}`} className={editingCommentIdRef.current === comment.commentId ? 'comment bg-gradient-to-r from-pink-900' : 'comment'}>
              ...
              {user && (user.id === comment.userId || user.isAdmin) && (
                <div className='comment-buttons'>
                    <button className="" onClick={() => handleEditComment(comment.commentId, comment.comment)}>
                    <MdEdit className='text-2xl'/>
                    </button>
                    <button className="" onClick={() => handleDeleteComment(comment.commentId)}>
