...
import { getReviewedMovies } from './userDataController.js';
...
export const getComments = async (req, res) => {
    ...
    try {
        const result = await session.executeRead(tx => tx.run('MATCH (u: User)-[r:COMMENTED]->(m: Movie {id: $movieId}) RETURN r, u', {movieId}));
        const data = result.records.map(record => { 
...
//             MATCH (u:User {userId: $userId})
//             MATCH (m:Movie {id: $movieId})
//             CREATE (u)-[r:COMMENTED {id: $commentId, comment: $comment, date: $date}]->(m)
//             RETURN r
//         `;
