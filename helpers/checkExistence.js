export const checkRelationshipExistence = async (session, startLabel, startProperty, startValue, relType, endLabel, endProperty, endValue) => {
    const query = `
...
export const checkNodeExistence = async (session, label, property, value) => {
    ...
    const result = await session.readTransaction(tx => tx.run(query, { value }));
