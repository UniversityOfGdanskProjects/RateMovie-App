        ...
        <h1 className='msg text-3xl'>Most Active Users</h1>
          <ul className='user-ranking'>
            ...
            {rankedUsers.slice(0, visibleUsers).map((userRanked, index) => (
              <li key={index}>
                <div>{index + 1}. {userRanked.username} {user && user.isAdmin && `(id: ${userRanked.userId})`}</div>
          ...
          {visibleUsers < rankedUsers.length && (
            <button className='big-btn m-auto my-3' onClick={handleLoadMore}>Load More</button>
          )}
        </>
      ) : (
        <LoadingPage />
